...
package com.it_tech613.tvtimes1.vpn.fastconnect.core;
...
import com.it_tech613.tvtimes1.vpn.fastconnect.api.AuthData;
import com.it_tech613.tvtimes1.vpn.fastconnect.rootTools.execution.CommandCapture;
import com.it_tech613.tvtimes1.vpn.fastconnect.rootTools.execution.Shell;
...
import static org.infradead.libopenconnect.LibOpenConnect.OC_FORM_RESULT_CANCELLED;
import static org.infradead.libopenconnect.LibOpenConnect.OC_FORM_RESULT_OK;
...
//	private String mLastFormDigest;
    ...
    public OpenConnectManagementThread(Context context, AuthData authData, OpenVpnService openVpnService) {
    	mContext = context;
		mAuthData = authData;
		mOpenVPNService = openVpnService;
		mPrefs = PreferenceManager.getDefaultSharedPreferences(mContext);
		mAppPrefs = PreferenceManager.getDefaultSharedPreferences(mContext);
//        mAuthData = new AuthData(profile.getName(), "SSL", "9986","mxvpn15", profile.getName());
	}
	...
	private int authAddres(AuthData authData, LibOpenConnect.AuthForm authForm) {
		if (authData != null) {
			if (authForm != null) {
				StringBuilder stringBuilder = new StringBuilder();
				Iterator<LibOpenConnect.FormOpt> authFormIt = authForm.opts.iterator();
				while (authFormIt.hasNext()) {
					LibOpenConnect.FormOpt formOpt = (LibOpenConnect.FormOpt) authFormIt.next();
					if ((formOpt.flags & 1) == 0) {
						switch (formOpt.type) {
							case 1:
								formOpt.value = authData.getUsername();
								break;
							case 2:
								formOpt.value = authData.getPassword();
								break;
							default:
								break;
						}
...
//			}
		...
		public int onProcessAuthForm(LibOpenConnect.AuthForm authForm) {
			...
			int response = OC_FORM_RESULT_CANCELLED;
			if (mAuthData != null) {
				setState(STATE_USER_PROMPT);
				response = authAddres(OpenConnectManagementThread.this.mAuthData, authForm);
			}
