...
import com.it_tech613.tvtimes1.ui.VpnActivity;
import com.it_tech613.tvtimes1.vpn.fastconnect.api.AuthData;
import com.it_tech613.tvtimes1.vpn.fastconnect.api.VpnConstant;
...
public class OpenVpnService extends VpnService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        String username = intent.getStringExtra(VpnConstant.username);
        String password = intent.getStringExtra(VpnConstant.password);
        String vpnname = intent.getStringExtra(VpnConstant.vpnName);
        String address = intent.getStringExtra(VpnConstant.address);
        String type = intent.getStringExtra(VpnConstant.type);

        if (mUUID == null) {
			return START_NOT_STICKY;
		}

        mAuthData = createAuthData(username, password, address, type, vpnname);
        if (mAuthData == null) {
            return START_NOT_STICKY;
        }
        killVPNThread(true);
        ...
        mVPN = new OpenConnectManagementThread(getApplicationContext(), mAuthData, this);
    ...
    public AuthData createAuthData(String username, String password, String address, String type, String vpnname) {
        Log.e("-------------username:", username);
        Log.e("-------------password:", password);
        Log.e("-------------address:", address);
        this.mAuthData = new AuthData(address, type, password, username, vpnname);
        return this.mAuthData;
    }

    public Builder getVpnServiceBuilder() {
        Builder b = new Builder();
        b.setSession(mAuthData.getVpnName());
